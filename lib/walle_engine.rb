module WalleEngine
  ...
  class WalleOne < WalleEngine
    ...
    def caclulate_potential_retentions(settling_period, date)
      ...
      @cards.each do |card|
        retentions_if_not_reviewed[card.id] = @history.card_retention_in(card.id, settling_period, date)
  ...
  class WalleTwo < WalleOne
    ...
    def generate_test(date,size=5)
      ...
      @cards.each do |card|
        ret = @history.card_retention_in(card.id, 0.day, date)
        if ret > 50
          healthy_cards << card
        else
  ...
  class WalleThree < WalleOne
    def generate_test(date,size=5)
      time_to_expire_cards=[]
      @cards.each do |card|
        time_to_expire_cards << [@history.card_optimal_date_of_review(card.id), card]
      end
      time_to_expire_cards.sort_by{|e| e[0]}[0..size-1].map{|e|e[1]}
